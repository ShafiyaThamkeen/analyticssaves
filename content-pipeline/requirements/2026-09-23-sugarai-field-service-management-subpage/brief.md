...
## 11. Post-delivery revision (2026-09-24) — supersedes parts of Sections 3, 4, 6 and 10.3 for Blocks 2 and 7

After delivery, the Content Writer reviewed the live page against two site-series reference pages (the Insurance template and an NGO/Foundations subpage) and identified that Blocks 2 ("Why SugarAI") and 7 ("Proven") had drifted from the site's established convention: those two bands are meant to carry short, SugarAI/SugarCRM-specific outcome claims, never market-size research or pricing data. This section records what changed and why, overriding the earlier Section 3/4/6/10.3 wording for these two blocks only. Everything else in the brief still stands.

1. **Why SugarAI band (Block 2):** replaced the market-size stats (US$809.8M MEA field service market, US$70.25B GCC facilities management, 64.88% outsourced FM) with the Sugar Sell product data sheet's own published customer-outcome figures: `+23%` Increased revenue, `+30%` Improved win rate, `3×` Pipeline volume. Source: https://sugarcrm-aws-marketplace-public-assets.s3.amazonaws.com/Sugar+Sell.pdf (©2024; the Plan agent read this PDF directly). These are framed honestly as general Sugar Sell (sales module) results, not claimed as field-service-specific — no field-service-specific SugarAI stat exists anywhere (confirmed by dedicated research, see `inputs/extracted/stats-research-v2.md`). No footnote markers on these three, matching the site's unfootnoted vendor-claim convention.
2. **Proven band (Block 7):** replaced the mixed industry-benchmark/market-data stats (Aquant first-time-fix/resolution figures, the 28.16% FSM revenue share, the 100% UAE broadband figure) with 3 honestly-sourced tiles: `-27%` Reduced case error rate and `+30%` Revenue contribution from support teams, both from the Sugar Serve data sheet (https://sugarcrm-aws-marketplace-public-assets.s3.amazonaws.com/Sugar+Serve.pdf, Rev. 08152022); and `8%` Higher recurring revenue with ERP integration, attributed inline on-page to Nucleus Research (https://nucleusresearch.com/research/single/how-unifying-crm-and-erp-with-sugar-drives-sales-performance/, 2025), since that one figure is third-party and needed visible attribution. Dropped to 3 tiles from 4 rather than stretch for an unsourced or off-topic fourth (the CSS `.proof-row` grid changed from 4 to 3 columns accordingly). The Block 7 H2 was changed from "What well-run field service operations achieve" to "What the service desk and ERP layers deliver" so it no longer implies the tiles are field-service-specific results, which they are not.
3. **Overrides earlier brief language:** Section 1's "do not name competitors or data sources in on-page copy" now excludes SugarCRM, Sugar Sell, Sugar Serve (the vendor and its own product names, not competitors) and Nucleus Research (an independent analyst firm, not a competitor) — naming them is required for honest attribution in these two bands. Section 10.3's Salesforce-stat replacement (28.16% / 100% broadband tiles) is superseded by this section.
4. **Known limitations, unresolved:** the `+30%` figure now appears in both bands (win rate in Block 2, support-team revenue contribution in Block 7) — different metrics, clearly labeled, but a skimming reader could mistake it for a repeat. The Sugar Serve data sheet is dated 2022 (~4 years old, older than the brief's 2-3 year preference) — it's the only Sugar Serve figure set found. The Nucleus 8% figure and both official sugarcrm.com/sugarai.com data sheet copies could not be directly fetched in this sandbox (egress blocked) and still need a human click-check before publication, along with every other source already flagged in Section 9.
